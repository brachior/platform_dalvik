...
public class StdAttributeFactory
        ...
        switch (context) {
            case CTX_CLASS: {
                ...
                if (name == AttBootstrapMethods.ATTRIBUTE_NAME) {
                    return bootstrap(cf, offset, length, observer);
                }
                break;
            }
            case CTX_FIELD: {
    ...
    private Attribute bootstrap(DirectClassFile cf, int offset, int length, ParseObserver observer) {
        ...
        BootstrapMethodList list = parseBootstrapMethods(
...
//            observer.parsed(bytes, offset, 2, "source: " + cst);
    ...
    private BootstrapMethodList parseBootstrapMethods(ByteArray bytes, ConstantPool pool, ParseObserver observer, int count) {
        ...
        try {
            for (int i = 0; i < count; i++) {
                ...
                BootstrapArgumentList bootstrapArgumentList = new BootstrapArgumentList(num_bootstrap_arguments);
