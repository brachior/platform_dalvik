...
public final class Type implements TypeBearer, Comparable<Type> {
	...
	static {
		/*
		 * Put all the primitive types into the intern table. This needs to happen before the array
		 * types below get interned.
		 */
		putIntern(BOOLEAN);
		putIntern(BYTE);
		putIntern(CHAR);
		...
		putIntern(SHORT);
		/*
		 * Note: VOID isn't put in the intern table, since it's special and shouldn't be found by a
		 * normal call to intern().
		 */
	}
	...
	public static final Type METHODTYPE = intern("Ljava/lang/invoke/MethodType;");

	/** {@code non-null;} instance representing {@code java.lang.invoke.MethodHandle} */
	public static final Type METHODHANDLE = intern("Ljava/lang/invoke/MethodHandle;");
	 ...
	 * sec4.3.2 for details on the field descriptor syntax. This method does <i>not</i> allow
	 * {@code "V"} (that is, type {@code void}) as a valid descriptor.
	 * 
	 * @param descriptor
	...
	public static Type intern(String descriptor) {
		...
		if (firstChar == '[') {
			/*
			 * Recursively strip away array markers to get at the underlying type, and build back on to
			 * form the result.
			 */
		...
		/*
		 * If the first character isn't '[' and it wasn't found in the intern cache, then it had better
		 * be the descriptor for a class.
		 */

		int length = descriptor.length();
		if ((firstChar != 'L') || (descriptor.charAt(length - 1) != ';')) { throw new IllegalArgumentException(
				"bad descriptor: " + descriptor); }

		/*
		 * Validate the characters of the class name itself. Note that vmspec-2 does not have a coherent
		 * definition for valid internal-form class names, and the definition here is fairly liberal: A
		 * name is considered valid as long as it doesn't contain any of '[' ';' '.' '(' ')', and it has
		 * no more than one '/' in a row, and no '/' at either end.
		 */
		...
		for (int i = 1; i < limit; i++) {
			...
			switch (c) {
				...
				case '/': {
					if ((i == 1) || (i == (length - 1)) || (descriptor.charAt(i - 1) == '/')) { throw new IllegalArgumentException(
							"bad descriptor: " + descriptor); }
					break;
				}
			}
	 ...
	 * this method is equivalent to calling {@code intern(name)} if {@code name} begins with
	...
	public static Type internClassName(String name) {
		...
		if (name.startsWith("[")) { return intern(name); }
	...
	private Type(String descriptor, int basicType, int newAt) {
		if (descriptor == null) { throw new NullPointerException("descriptor == null"); }

		if ((basicType < 0) || (basicType >= BT_COUNT)) { throw new IllegalArgumentException(
				"bad basicType"); }

		if (newAt < -1) { throw new IllegalArgumentException("newAt < -1"); }
	...
	public boolean equals(Object other) {
		if (this == other) {
			/*
			 * Since externally-visible types are interned, this check helps weed out some easy cases.
			 */
			return true;
		}
	...
	public String toHuman() {
		...
		if (isArray()) { return getComponentType().toHuman() + "[]"; }
	...
	public String getClassName() {
		if (className == null) {
			if (!isReference()) { throw new IllegalArgumentException("not an object type: " + descriptor); }
	 ...
	 * that is not a return address. This method is just convenient shorthand for
	...
	/**
	 * Gets the bytecode index at which this uninitialized type was allocated. This returns
	 * {@code Integer.MAX_VALUE} if this type is an uninitialized incoming parameter (i.e., the
	 * {@code this} of an {@code <init>} method) or {@code -1} if this type is in fact
	 * <i>initialized</i>.
	 * 
	...
	public Type getInitializedType() {
		if (initializedType == null) { throw new IllegalArgumentException("initialized type: "
				+ descriptor); }
	...
	public Type getComponentType() {
		if (componentType == null) {
			if (descriptor.charAt(0) != '[') { throw new IllegalArgumentException("not an array type: "
					+ descriptor); }
	...
	public Type asUninitialized(int newAt) {
		if (newAt < 0) { throw new IllegalArgumentException("newAt < 0"); }

		if (!isReference()) { throw new IllegalArgumentException("not a reference type: " + descriptor); }

		if (isUninitialized()) {
			/*
			 * Dealing with uninitialized types as a starting point is a pain, and it's not clear that
			 * it'd ever be used, so just disallow it.
			 */
			throw new IllegalArgumentException("already uninitialized: " + descriptor);
		}

		/*
		 * Create a new descriptor that is unique and shouldn't conflict with "normal" type descriptors
		 */
